...
<body class="k">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($check_result->num_rows > 0) {
            // E-posta adresi zaten kayıtlı ise JavaScript alert ile uyarı mesajı göster
            echo "<script>alert('This email is already registered.');</script>";
        } else {
            // E-posta adresi benzersizse yeni kullanıcıyı kaydet
            $sql = "INSERT INTO kullanıcılar (name, surname, mail, password) VALUES ('$name', '$surname', '$email', '$hashed_password')";
            if ($conn->query($sql) === TRUE) {
                // Kayıt başarılı olduysa JavaScript alert ile uyarı mesajı göster
                echo "<script>alert('New record created successfully');</script>";
                header("Location: signin.php");
            } else {
                // Kayıt başarısız olduysa JavaScript alert ile hata mesajı göster
